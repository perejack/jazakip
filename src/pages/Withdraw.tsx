...
const Withdraw = () => {
  ...
  const [checkingWithdrawRules, setCheckingWithdrawRules] = useState(true);
  const [withdrawalsAfterActivation, setWithdrawalsAfterActivation] = useState(0);
  ...
  const needsUpgradeForFullWithdrawal = !hasActivePackage && withdrawalsAfterActivation >= 1;
  ...
  useEffect(() => {
    const checkWithdrawRules = async () => {
      if (!profile?.id) {
        setHasActivePackage(false);
        setWithdrawalsAfterActivation(0);
        setCheckingWithdrawRules(false);
        return;
      }

      setCheckingWithdrawRules(true);
      ...
      if (packageError) {
        console.error("Error checking active package:", packageError);
        setHasActivePackage(false);
      } else {
        setHasActivePackage(Boolean(packageData?.length));
      }
      ...
      const { data: activationData, error: activationError } = await supabase
        ...
        .select("unlocked_at")
        .eq("user_id", profile.id)
        .eq("category_id", "activation")
        .eq("payment_status", "completed")
        .single();

      if (activationError || !activationData?.unlocked_at) {
        setWithdrawalsAfterActivation(0);
        setCheckingWithdrawRules(false);
        return;
      }

      const { count, error: withdrawalCountError } = await supabase
        .from("withdrawals")
        .select("id", { count: "exact", head: true })
        .eq("user_id", profile.id)
        .gte("created_at", activationData.unlocked_at);

      if (withdrawalCountError) {
        console.error("Error checking withdrawals after activation:", withdrawalCountError);
        setWithdrawalsAfterActivation(0);
      } else {
        setWithdrawalsAfterActivation(count || 0);
      }

      setCheckingWithdrawRules(false);
    };

    void checkWithdrawRules();
  }, [profile?.id]);

  const handleWithdraw = () => {
    if (needsUpgradeForFullWithdrawal) {
      toast.error("Upgrade your package to withdraw your full amount.");
            ...
            <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="bg-card rounded-2xl p-5 shadow-card">
                ...
                <button
                  ...
                  disabled={!canWithdraw || submitting || checkingWithdrawRules}
                  className={`w-full mt-5 rounded-xl py-4 font-bold text-sm transition ${
                    canWithdraw && !submitting && !checkingWithdrawRules
                      ? "bg-primary text-primary-foreground shadow-premium hover:opacity-90"
                      : "bg-muted text-muted-foreground cursor-not-allowed"
                  }`}
                >
                  {checkingWithdrawRules ? 'Checking account...' : submitting ? 'Processing...' : 'Withdraw to M-Pesa'}
                </button>
              </div>
            </motion.div>
          )}
        ...
        <DialogContent className="sm:max-w-md p-0 overflow-hidden bg-card border-0">
          <div className="p-6">
            ...
            <p className="text-muted-foreground text-sm text-center mb-6">
              Your account has been activated, but you need to upgrade your package to withdraw your full amount to M-Pesa.
            </p>
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 rounded-xl py-3 bg-primary text-primary-foreground font-bold text-sm shadow-premium"
              >
                Upgrade now
              </button>
            </div>
