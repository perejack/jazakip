...
// Configuration via environment variables (set these in Vercel)
const SWIFTPAY_BASE_URL = process.env.SWIFTPAY_BASE_URL || 'https://swiftpay-backend-uvv9.onrender.com';
  ...
  try {
    if (!SWIFTPAY_API_KEY || !SWIFTPAY_TILL_ID) {
      return res.status(500).json({
        success: false,
        error: 'Server misconfiguration: missing SWIFTPAY_API_KEY or SWIFTPAY_TILL_ID',
      });
    }
